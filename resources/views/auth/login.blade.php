...
<div class="mx-auto flex max-w-6xl flex-col items-center px-4 py-16 sm:px-6 sm:py-24">
    ...
    <p class="mt-3 max-w-md text-center text-muted">Vul je e-mailadres in. Je krijgt een code van 6 cijfers om in te loggen.</p>

    <form method="POST" action="{{ route('login.send') }}" class="mt-10 w-full max-w-md rounded-[28px] border border-line bg-card p-6 sm:p-8">
        @csrf
        ...
        @enderror
        <button type="submit" class="mt-8 w-full rounded-full bg-ink px-5 py-3 text-sm font-semibold text-paper transition hover:opacity-90">Stuur inlogcode</button>
    </form>
